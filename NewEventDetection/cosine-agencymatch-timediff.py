...
with open('event1.txt','r') as openfileobject:
    for line in openfileobject:
		...
		khabarID.append(x[0])
		publish_time.append(x[9]+"-"+x[10]+"-"+x[11]+"-"+x[13]+"-"+x[12])
...
for num in range(1,len(all_documents)):
	if khabarID[num] not in off_toppic_index_list:
		...
		print khabarID[num],max_similarity,('1' if  khabargozari[num]==khabargozari[max_index] else '0'),time_diff,khabarID[max_index],max_index
		max_similarity=0
	#else:
	#	print "off-topic"
