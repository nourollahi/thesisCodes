...
patt = re.compile('(\s*)\n(\s*)')
with open('OldNewPlain.txt', 'r') as openfileobject:
    for line in openfileobject:
        line = patt.sub('', line)
        x = re.split('\t', line)
        if str(x[0]) not in off_toppic_index_list:
                all_documents.append(str(x[5]))
                khabarID.append(x[0])
openfileobject.close()
...
model = KeyedVectors.load_word2vec_format('/home/mostafa/Desktop/WMD/wiki.fa.vec', binary=False)
